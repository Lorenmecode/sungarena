...
def get_db():
    if DATABASE_URL:
        if psycopg is None:
            raise RuntimeError("Cần cài đặt psycopg để kết nối PostgreSQL.")
    ...
    with get_db() as connection:
        ...
        for path in VIDEO_FOLDER.glob("*"):
            if path.is_file() and is_allowed_video(path.name):
                exists = execute(
                    connection,
                    "SELECT 1 FROM media WHERE filename = %s LIMIT 1",
                    (path.name,),
                ).fetchone()
                if exists is None:
                    execute(
...
def serve_media(media_id):
    with get_db() as connection:
        media = execute(
            connection,
            "SELECT data, content_type, filename FROM media WHERE id = %s",
            (media_id,),
        ).fetchone()
    if media is None:
